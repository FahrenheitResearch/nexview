...
static RUSTMAPS_RENDERER: OnceLock<Option<rustmaps::render::TileRenderer>> = OnceLock::new();
static RUSTMAPS_WARNED: AtomicBool = AtomicBool::new(false);

fn get_rustmaps_renderer() -> Option<&'static rustmaps::render::TileRenderer> {
    RUSTMAPS_RENDERER.get_or_init(|| {
        ...
        match candidates.into_iter().find(|d| d.exists()) {
            Some(data_dir) => {
                log::info!("Loading rustmaps geodata from: {}", data_dir.display());
                match rustmaps::load_renderer(&data_dir) {
                    Ok(r) => Some(r),
                    Err(e) => {
                        log::error!("Failed to load rustmaps geodata: {}", e);
                        None
                    }
                }
            }
            None => {
                log::warn!("rustmaps_data directory not found — place it next to the executable for offline map tiles");
                None
            }
        }
    }).as_ref()
...
impl MapTileManager {
    ...
    pub fn request_tile(&self, key: TileKey) {
        ...
        self.runtime.spawn_blocking(move || {
            let renderer = match get_rustmaps_renderer() {
                Some(r) => r,
                None => {
                    if !RUSTMAPS_WARNED.swap(true, Ordering::Relaxed) {
                        log::warn!("Skipping map tile render — rustmaps_data not available");
                    }
