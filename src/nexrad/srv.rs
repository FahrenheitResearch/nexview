...
impl SRVComputer {
    ...
    pub fn estimate_storm_motion(velocity_sweeps: &[&Level2Sweep]) -> (f32, f32) {
        ...
        // Compute mean wind (u, v) for a single sweep, filtering gates < 20km
        // and weighting by range to better capture environmental flow.
        ...
        if level_winds.len() >= 3 {
            ...
            let low_u: f64 =
                level_winds[..low_count].iter().map(|(_, u, _)| u).sum::<f64>() / low_count as f64;
            let low_v: f64 =
                level_winds[..low_count].iter().map(|(_, _, v)| v).sum::<f64>() / low_count as f64;
            ...
            if shear_mag > 0.5 {
                ...
                let dev_u = mean_u_all + dev_magnitude * (shear_v / shear_mag);
                let dev_v = mean_v_all + dev_magnitude * (-shear_u / shear_mag);

                // Convert velocity in kts (data is already in kts from NEXRAD)
                let speed_kts = (dev_u * dev_u + dev_v * dev_v).sqrt();
                // Direction storm is MOVING TOWARD, then convert to FROM
                let dir_rad = dev_u.atan2(dev_v);
        ...
        let speed_kts = (mean_u * mean_u + mean_v * mean_v).sqrt();
