...
use crate::nexrad::level2::{RadialData, MomentData};
    ...
    pub fn compute_vil(file: &Level2File) -> Level2Sweep {
        ...
        let mut ref_sweeps: Vec<&Level2Sweep> = file
            ...
            .filter(|s| {
                s.radials
                    .first()
                    .map(|r| r.moments.iter().any(|m| m.product == RadarProduct::Reflectivity))
                    .unwrap_or(false)
            })
            .collect();
        ...
        for radial in template_radials {
            let ref_moment = match radial.moments.iter().find(|m| m.product == RadarProduct::Reflectivity) {
                Some(m) => m,
                None => continue,
            };
            ...
            for gate_idx in 0..num_gates {
                ...
                if tilt_values.len() < 2 {
                    vil_data[gate_idx] = if tilt_values.is_empty() { f32::NAN } else { 0.0 };
                    continue;
                }
