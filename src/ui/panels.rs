...
impl SidePanel {
            ...
            egui::ComboBox::from_id_salt("color_preset")
                ...
                .show_ui(ui, |ui| {
                    for (name, sel) in app.color_table_manager.available_names() {
                        let is_selected = app.color_table_manager.selection_for(app.selected_product) == sel; if ui.selectable_label(is_selected, &name).clicked() {
    ...
    fn animation_controls(app: &mut RadarApp, ui: &mut Ui) {
        ...
        ui.horizontal(|ui| {
            if app.anim_loading {
                ui.spinner();
                ui.label(format!("Loading {}/{}...", app.anim_received_count, app.anim_download_queue.len()));
    ...
    fn color_bar(app: &mut RadarApp, ui: &mut Ui) {
        let color_table = app.color_table_manager.resolve(app.selected_product);
